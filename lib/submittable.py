...
class Submittable:
    ...
    def submitInternalFormResponse(self, submission_id, primary_unique_id,
        ...
        if response.status_code != 201:
            if response.status_code == 429:
                logger.info("submit internal form - api rate limit hit wait 15 mins")
                # self.event.wait(900)
                time.sleep(900)
    ...
    def getListOfSubmissions(self):
        ...
        page_size = 50
        # total_pages = 505
        endpoint = f'{self.baseURL}/submissions?projects.include={config.project_id_1}&projects.include={config.project_id_2}&projects.include={config.project_id_3}&projects.include={config.project_id_4}&statuses.include=new&statuses.include=in_progress&pageSize={page_size}'
        headers = {'Content-type': 'application/json'}
        response = requests.get(endpoint, auth=("", self.api_key), headers=headers)
        if response.status_code != 200:
            if response.status_code == 429:
        ...
        for page in range(0, total_pages):
            ...
            if page == total_pages:
                break
            nextPage = page + 1
            endpoint = f'{self.baseURL}/submissions?projects.include={config.project_id_1}&projects.include={config.project_id_2}&projects.include={config.project_id_3}&projects.include={config.project_id_4}&statuses.include=new&statuses.include=in_progress&page={nextPage}&pageSize={page_size}'
            headers = {'Content-type': 'application/json'}
            response = requests.get(endpoint, auth=("", self.api_key), headers=headers)
            if response.status_code != 200:
                if response.status_code == 429:
